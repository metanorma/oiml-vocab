...
# OIML Complete Vocabulary ↔ G 18:202X cross-reference + tip dataset builder.
#
# Outputs a permanent report directory at
#   reference-docs/oiml-complete-vs-g18/
...
#   05-no-family-pub-in-corpus.txt        G 18 cites a pub whose family IS in OIML Complete Vocabulary but
...
G18_COMPLETE_CONCEPTS = REPO_ROOT + "datasets" + "oiml-complete" + "concepts"
...
OUT_DIR = REPO_ROOT + "reference-docs" + "oiml-complete-vs-g18"
...
module OimlCompleteVsG18
  ...
  # ------- OIML Complete Vocabulary loading -------
  ...
  def cross_reference(g18_dyn, g18)
    ...
    g18.each_value do |entry|
      ...
      # Case 3: family not in OIML Complete Vocabulary data
      unless tip[family]
  ...
  def build_g18_current_tip_dataset(g18_dyn)
    out_dir = REPO_ROOT + "datasets" + "oiml-complete"
    ...
    register = {
      ...
      "derived" => true,
      "derived_from" => "oiml-complete tip concepts (correlated across languages)",
  ...
  def write_reports(g18_dyn, g18, xref)
    ...
    File.write(OUT_DIR + "03-below-tip-no-clause.txt",
               xref[:below_tip_no_clause].map { |e|
                 ahead = e[:ahead] ? " [AHEAD: G18 newer than OIML Complete Vocabulary tip]" : ""
    ...
    lines = []
    lines << "OIML Complete Vocabulary concepts missing from G 18:202X — grouped by family"
    lines << "=" * 60
    lines << ""
    ...
    # 08 summary
    summary = []
    summary << "OIML Complete Vocabulary ↔ G 18:202X cross-reference summary"
    summary << "=" * 60
    summary << ""
    summary << "G 18:202X entries:        #{xref[:g18_entries_total]}"
    summary << "OIML Complete Vocabulary unique tip concepts:  #{g18_dyn[:tip_concepts].size}"
    summary << ""
    ...
    summary << "  Family in OCR corpus, no OIML Complete Vocabulary concepts:     #{xref[:family_in_corpus_no_concepts].size}  ← confirmed no terms OR extraction gap"
    ...
    summary << ""
    summary << "OIML Complete Vocabulary concepts missing from G 18:        #{xref[:tip_missing_from_g18].size}"
    summary << ""
...
g18_dyn = OimlCompleteVsG18.load_g18_current_tip
g18 = OimlCompleteVsG18.load_g18
xref = OimlCompleteVsG18.cross_reference(g18_dyn, g18)
OimlCompleteVsG18.write_reports(g18_dyn, g18, xref)

puts "OIML Complete Vocabulary ↔ G 18:202X cross-reference"
puts "=" * 60
puts
puts "G 18:202X entries:        #{xref[:g18_entries_total]}"
puts "OIML Complete Vocabulary unique tip concepts:  #{g18_dyn[:tip_concepts].size}"
puts
...
puts "  Family in OCR corpus, no OIML Complete Vocabulary concepts:     #{xref[:family_in_corpus_no_concepts].size}"
...
puts
puts "OIML Complete Vocabulary concepts missing from G 18:        #{xref[:tip_missing_from_g18].size}"
puts
...
if options[:build]
  ...
  result = OimlCompleteVsG18.build_g18_current_tip_dataset(g18_dyn)
  puts "  Wrote #{result[:written]} concepts to #{result[:out_dir]}"
  puts "  Validating..."
  system("ruby #{REPO_ROOT + 'scripts' + 'validate_datasets.rb'} --datasets oiml-complete")
end
